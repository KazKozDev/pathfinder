...
const ClipboardIcon = () => (
  <Icon path="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2m4-2h4a2 2 0 0 1 2 2v2H8V4a2 2 0 0 1 2-2z" />
);
const CalendarIcon = () => (
  <Icon path="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2z" />
);
const ContactsIcon = () => (
  <Icon path="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2 M9 3a4 4 0 1 0 0 8 4 4 0 0 0 0-8z M23 21v-2a4 4 0 0 0-3-3.87" />
);
const SparkleIcon = () => (
  <Icon path="M10 3L8 8l-5 2 5 2 2 5 2-5 5-2-5-2-2-5z M18 13l-2 4-4 2 4 2 2 4 2-4 4-2-4-2-2-4z" />
);
const ResumesIcon = () => (
  <Icon path="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zM14 2v6h6M16 13H8M16 17H8M10 9H8" />
);
const SettingsIcon = () => (
...
const PaperclipIcon = () => (
  <Icon path="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48" />
);
const SunIcon = () => (
  <Icon path="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42 M12 16a4 4 0 100-8 4 4 0 000 8z" />
);
const MoonIcon = () => (
...
const ClipboardCheckIcon = () => (
  <Icon path="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2m4-2h4a2 2 0 0 1 2 2v2H8V4a2 2 0 0 1 2-2zM9 14l2 2 4-4" />
);
...
const PdfIcon = () => (
  <Icon path="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zM9.5 14.5v-5h1a1.5 1.5 0 0 1 0 3h-1m-2.5-3h1v5H7V9.5c0-.83.67-1.5 1.5-1.5H10v5h1.5v-5a1.5 1.5 0 0 1 1.5-1.5H16v5h1.5V8h-3v1.5h1.5v2h-1.5V8H13c-.83 0-1.5.67-1.5 1.5v5H9.5z" />
);
const WordIcon = () => (
  <Icon path="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zM15 18h-2l-1-4-1 4H9l-2-8h2l1 4 1-4h2l1 4 1-4h2l-2 8z" />
);
const LinkedInIcon = () => (
...
const MicrophoneIcon = () => (
  <Icon path="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3zM19 10v2a7 7 0 0 1-14 0v-2H3v2a8 8 0 0 0 7 7.93V22h2v-2.07A8 8 0 0 0 21 12v-2h-2z" />
);
...
const availableTools: Tool[] = [
  {
    id: 'google_search',
    name: 'Google Search',
    description: 'Access up-to-date information from the web.',
  },
  {
    id: 'linkedin_analysis',
    name: 'LinkedIn Analysis',
    description: 'Analyze profiles and company pages on LinkedIn.',
  },
];

// Function to load prompts from files
const loadPromptsFromFiles = async (): Promise<{
  coverLetter: string;
  resumeChecker: string;
}> => {
  try {
...
Begin the interview now.`,
  ...
  agents: {
    recruiter: {
      ...
      prompt:
        'Specializes in HR processes, resume optimization, and negotiation strategies.',
      tools: {
    ...
    research: {
      name: 'Research Agent',
      prompt:
        'Conducts deep analysis of companies, industries, and interviewers.',
      tools: {
    ...
    coach: {
      name: 'Coach Agent',
      prompt:
        'Acts as a personal career coach, helping with interview practice, salary negotiation, and long-term career planning.',
      tools: {
        google_search: { enabled: false },
...
const JobDetailModal = ({
  ...
  const handleGenerateLetter = async () => {
    ...
    const selectedResume = resumes.find(
      r => r.id === formData.selectedResumeId
    );

    if (!selectedResume) {
      setFormData(prev => ({
        ...prev,
        coverLetter: 'Error: Please select an associated resume first.',
      }));
    ...
    if (!formData.description || formData.description.trim().length < 50) {
      setFormData(prev => ({
        ...prev,
        coverLetter:
          'Error: Job description is too short or missing. Please add a detailed job description first.',
      }));
      setIsGenerating(false);
      return;
    }
    ...
    if (!selectedResume.summary || selectedResume.summary.trim() === '') {
      setFormData(prev => ({
        ...prev,
        coverLetter:
          'Error: Resume summary is missing. Please add a summary to your resume first.',
      }));
      setIsGenerating(false);
      return;
    }
    ...
    if (!selectedResume.experience || selectedResume.experience.length === 0) {
      setFormData(prev => ({
        ...prev,
        coverLetter:
          'Error: Resume has no work experience. Please add work experience to your resume first.',
      }));
      setIsGenerating(false);
      return;
    }

    const contact = formData.contactIds?.[0]
      ? crmContacts.find(c => c.id === formData.contactIds[0])
      : null;
    ...
    try {
      ...
      if (response.text && response.text.trim().length > 0) {
        ...
        setFormData(prev => ({
          ...prev,
          coverLetter:
            'Error: Generated cover letter is empty. Please try again.',
        }));
      }
    } catch (error) {
  ...
  const removeLog = (id: number) => {
    setFormData(prev => ({
      ...prev,
      communicationLog: prev.communicationLog?.filter(l => l.id !== id),
    }));
  };
      ...
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        <header className="modal-header">
          ...
          <div className="modal-tabs">
            {['Overview', 'Preparation', 'Contacts & Logs', 'Metadata'].map(
              tabName => (
                <button
                  key={tabName}
        ...
        <div className="modal-body" ref={modalBodyRef}>
          ...
          {activeTab === 'Preparation' && (
            <div className="form-grid two-cols">
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={e =>
                    setFormData(p => ({
                      ...p,
                      selectedResumeId: parseInt(e.target.value),
                    }))
                  }
                >
              ...
              <div className="form-group full-width">
                <label>Salary Range (from posting)</label>
                <input
                  name="range"
                  value={formData.salaryInfo?.range}
                  onChange={e => handleNestedChange(e, 'salaryInfo', 'range')}
                />
              </div>
              <div className="form-group full-width">
                ...
                <input
                  name="expectations"
                  value={formData.salaryInfo?.expectations}
                  onChange={e =>
                    handleNestedChange(e, 'salaryInfo', 'expectations')
                  }
                />
              </div>
              <div className="form-group full-width">
                <label>Discussed/Offered Salary</label>
                <input
                  name="discussed"
                  value={formData.salaryInfo?.discussed}
                  onChange={e =>
                    handleNestedChange(e, 'salaryInfo', 'discussed')
                  }
                />
              </div>
              <div className="form-group full-width">
                <label>Questions for Interviewer</label>
                <textarea
                  name="questionsForInterviewer"
                  value={formData.questionsForInterviewer}
                  onChange={handleChange}
                  rows={6}
                ></textarea>
              </div>
          ...
          {activeTab === 'Contacts & Logs' && (
            <div>
              <div className="list-widget">
                ...
                <select
                  onChange={e => addContactId(Number(e.target.value))}
                  value=""
                >
                ...
                <p className="placeholder-text">
                  Manage all contacts in the 'Network' tab.
                </p>
              </div>
              <div className="list-widget">
                <h3>
                  Communication Log <button onClick={addLog}>+ Add</button>
                </h3>
                <div className="list-container">
                  {formData.communicationLog?.map((log, index) => (
                    <div key={log.id} className="list-item-form log-item">
                      ...
                      <select
                        ...
                        onChange={e =>
                          updateLog(
                            index,
                            'type',
                            e.target.value as LogEntry['type']
                          )
                        }
                      >
                        <option>Email</option>
                      ...
                      <textarea
                        ...
                        value={log.summary}
                        onChange={e =>
                          updateLog(index, 'summary', e.target.value)
                        }
                        rows={2}
                      ></textarea>
                      <button
                  ...
                  {(!formData.communicationLog ||
                    formData.communicationLog.length === 0) && (
                    <p className="placeholder-text">
          ...
          {activeTab === 'Metadata' && (
            <div className="form-grid two-cols">
              ...
              <div className="form-group">
                ...
                <StarRating
                  rating={formData.interestLevel || 0}
                  setRating={r =>
                    setFormData(p => ({ ...p, interestLevel: r }))
                  }
                />
              </div>
              <div className="form-group full-width">
    ...
    <nav className="sidebar">
      <div>
        <div className="sidebar-header">
          <h1>
            <span style={{ color: '#D4AF37' }}>Path</span>finder
          </h1>
        </div>
...
const DashboardView = ({
  ...
  const [nextActions, setNextActions] = useState<
    { suggestion_text: string; action_type: string; job_id?: number }[]
  >([]);
  ...
  const recentApplications = jobs.filter(
    j =>
      (j.applicationDate &&
        new Date(j.applicationDate).getTime() >= oneWeekAgo) ||
      (j.status === 'Applied' && j.dateAdded >= oneWeekAgo)
  ).length;
  const goalProgress =
    settings.profile.weeklyGoal > 0
      ? (recentApplications / settings.profile.weeklyGoal) * 100
      : 0;
  ...
  const handleGenerateNextActions = async () => {
    ...
    const jobsSummary = jobs
      .map(
        j =>
          `- (ID: ${j.id}) ${j.title} at ${j.company} (Status: ${j.status}, Applied: ${j.applicationDate || 'N/A'}, Next Step Date: ${j.nextStepDate || 'N/A'})`
      )
      .join('\n');
    ...
    const successRate =
      totalJobs > 0
        ? (((interviewingJobs + offerJobs) / totalJobs) * 100).toFixed(1)
        : '0';
    const avgResponseTime =
      jobs.filter(j => j.applicationDate && j.nextStepDate).length > 0
        ? jobs
            .filter(j => j.applicationDate && j.nextStepDate)
            .map(
              j =>
                new Date(j.nextStepDate!).getTime() -
                new Date(j.applicationDate!).getTime()
            )
            .reduce((a, b) => a + b, 0) /
          jobs.filter(j => j.applicationDate && j.nextStepDate).length /
          (1000 * 60 * 60 * 24)
        : 0;

    // Get user's top skills for market analysis
    const userSkills = settings.profile.masterSkills
    ...
    try {
            ...
            items: {
              ...
              properties: {
                suggestion_text: {
                  type: Type.STRING,
                  description:
                    'The suggested action for the user with market insights and motivational elements.',
                },
                action_type: {
                  type: Type.STRING,
                  description:
                    'The type of action. Must be one of: PREPARE, FOLLOW_UP, APPLY, REVIEW, GOAL, SALARY_RESEARCH.',
                },
                job_id: {
                  type: Type.NUMBER,
                  description:
                    'The ID of the job this action relates to. Omit if not applicable.',
                },
              },
      ...
      setNextActions([
        {
          suggestion_text: 'Could not generate suggestions. Try refreshing.',
          action_type: 'ERROR',
        },
      ]);
    } finally {
  ...
  const handleAnalyzeSkills = async () => {
    ...
    if (
      ...
      setAnalysisResult(
        'Please select a job with a description and ensure your skills are listed in Settings.'
      );
      return;
    }
    setIsAnalyzing(true);
...
**MISSING SKILLS:**`;
  ...
  const upcomingDeadlines = jobs
    ...
    .sort(
      (a, b) =>
        new Date(a.nextStepDate!).getTime() -
        new Date(b.nextStepDate!).getTime()
    )
    .slice(0, 3);
    ...
    <div className="dashboard-view">
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <p>
            {
              jobs.filter(j => ['Interviewing', 'Test Task'].includes(j.status))
                .length
            }
          </p>
        </div>
        ...
        <div className="stat-card rejection-card">
          <h3>Rejections</h3>
          <p>{jobs.filter(j => j.status === 'Rejection').length}</p>
        </div>
      </div>
      ...
      <div className="dashboard-main-grid">
        <div className="widget goal-widget">
          ...
          <div className="goal-progress">
            ...
            <div className="goal-text">
              <span className="current">{recentApplications}</span> /{' '}
              <span className="total">{settings.profile.weeklyGoal}</span>{' '}
              Applied
            </div>
          </div>
            ...
            <input
              ...
              onChange={e =>
                setSettings({
                  ...
                  profile: {
                    ...settings.profile,
                    weeklyGoal: parseInt(e.target.value, 10) || 0,
                  },
                })
              }
            />
        ...
        <div className="widget skill-analysis-widget">
          ...
          <div className="skill-analysis-controls">
            ...
            <button
              ...
              disabled={isAnalyzing || !analysisJobId}
            >
          ...
          <div className="analysis-results">
            {isAnalyzing && (
              <p className="placeholder-text">AI is analyzing...</p>
            )}
            {analysisResult && (
              <pre className="generated-text">{analysisResult}</pre>
            )}
            {!isAnalyzing && !analysisResult && (
              <p className="placeholder-text">
                Select a job and click Analyze to see missing skills.
              </p>
            )}
          </div>
        </div>
          ...
          <ul>
            {recentJobs.length > 0 ? (
              recentJobs.map(job => (
                <li key={job.id} onClick={() => onJobDoubleClick(job)}>
                  <span
                    className={`status-dot status-dot--${job.status.replace(' ', '-')}`}
                  ></span>
        ...
        <div className="widget next-actions-widget">
          ...
          <ul>
            {isLoadingActions && (
              <p className="placeholder-text">Generating suggestions...</p>
            )}
            {!isLoadingActions && nextActions.length === 0 && (
              <p className="placeholder-text">
                No suggestions right now. Add more jobs!
              </p>
            )}
            {nextActions.map((action, index) => {
              const relatedJob = action.job_id
                ? jobs.find(j => j.id === action.job_id)
                : null;
              return (
                <li key={index} className="action-item">
                  ...
                  <div className="action-text">
                    ...
                    {relatedJob && (
                      <a
                        className="action-job-link"
                        onClick={() => onJobDoubleClick(relatedJob)}
                      >
          ...
          <div className="form-grid two-cols">
            <div className="form-group full-width">
              ...
              <input
                ...
                value={formData.title}
                onChange={handleChange}
                placeholder="e.g., Interview with TechCorp"
              />
            </div>
            <div className="form-group">
              <label>Date</label>
...
const CalendarView = ({
  ...
  const eventsByDate: {
    [key: string]: {
      type: 'job-application' | 'job-nextStep' | 'custom';
      data: Job | CalendarEvent;
    }[];
  } = {};
  jobs.forEach(job => {
  ...
  Object.keys(eventsByDate).forEach(date => {
    eventsByDate[date].sort((a, b) => {
      const timeA =
        a.type === 'custom' ? (a.data as CalendarEvent).time : undefined;
      const timeB =
        b.type === 'custom' ? (b.data as CalendarEvent).time : undefined;
      ...
      <div className="calendar-grid">
        ...
        {days.map((d, i) => {
          ...
          return (
            <div
              key={i}
              className={`calendar-day ${d.getMonth() !== currentDate.getMonth() ? 'other-month' : ''} ${isToday(d) ? 'today' : ''}`}
              ...
              <div className="day-events">
                {events.map(({ type, data }, index) => {
                  if (type.startsWith('job')) {
                    ...
                    const eventType =
                      type === 'job-application' ? 'application' : 'nextStep';
                    const eventText =
                    ...
                    return (
                      <div
                        ...
                        className={`calendar-event job ${eventType}`}
                        onClick={e => {
                          e.stopPropagation();
                          onJobClick(job);
                        }}
                    ...
                    const contact = event.contactId
                      ? crmContacts.find(c => c.id === event.contactId)
                      : null;
                    return (
                      <div
                        key={event.id}
                        className={`calendar-event custom ${event.type.toLowerCase()}`}
                        onClick={e => {
                          e.stopPropagation();
                          onEventClick(event);
                        }}
                      >
                        ...
                        {contact && (
                          <span className="event-contact-name">
                            {contact.name}
                          </span>
                        )}
                      </div>
...
const ContactEditor = ({
  ...
  contact: CrmContact;
  jobs: Job[];
  onUpdate: (updatedContact: CrmContact) => void;
  onJobClick: (job: Job) => void;
}) => {
      ...
      <div className="form-grid two-cols">
        ...
        <div className="form-group full-width">
          ...
          <div className="tag-input-container">
            {formData.tags?.map(tag => (
              <div key={tag} className="tag-pill">
                {tag} <button onClick={() => removeTag(tag)}>&times;</button>
              </div>
            ))}
            <input
              value={tagInput}
      ...
      <div className="widget linked-jobs-widget">
        ...
        <ul>
          {linkedJobs.length > 0 ? (
            ...
            <p className="placeholder-text">
              This contact is not yet linked to any job.
            </p>
          )}
        </ul>
      </div>
...
const ContactsView = ({
  ...
  const [selectedContactId, setSelectedContactId] = useState<number | null>(
    crmContacts.length > 0 ? crmContacts[0].id : null
  );
  ...
  const handleDeleteContact = () => {
    ...
    if (
      window.confirm(
        'Are you sure you want to delete this contact? This will also remove them from any linked jobs.'
      )
    ) {
  ...
  const filteredContacts = crmContacts
    .filter(
      contact =>
      ...
      <div className="contact-list-panel">
        ...
        <ul className="contact-list">
          {filteredContacts.map(contact => (
            <li
              key={contact.id}
              className={`contact-list-item ${contact.id === selectedContactId ? 'active' : ''}`}
          ...
          {filteredContacts.length === 0 && (
            <p className="placeholder-text">No contacts found.</p>
          )}
        </ul>
      ...
      <div className="contact-detail-panel">
        {selectedContact ? (
          ...
          <div className="placeholder-container">
            ...
            <p className="placeholder-text">
              Create a new contact or select one to view their details.
            </p>
          </div>
        )}
      </div>
...
const GeneralAssistantTool = ({
  ...
  messages: Message[];
  isLoading: boolean;
  onSendMessage: (input: string, file?: File) => Promise<void>;
  isAgentMixMode: boolean;
  setIsAgentMixMode: React.Dispatch<React.SetStateAction<boolean>>;
}) => {
      ...
      <div className="chat-input-area">
        ...
        <form className="chat-input" onSubmit={localHandleSendMessage}>
          ...
          <textarea
            ...
            placeholder={
              selectedFile
                ? 'Describe what to do with the file...'
                : 'Ask for interview prep for TechCorp...'
            }
...
const VoiceInterviewTool = ({
  ...
  useEffect(() => {
    ...
    const handleVoicesChanged = () => {
      ...
      if (englishVoices.length > 0) {
        ...
        const microsoftVoice = englishVoices.find(
          v => v.name.includes('David') || v.name.includes('Zira')
        );
        const appleVoice = englishVoices.find(
          v => v.name.includes('Siri') || v.name.includes('Alex')
        );
    ...
    recognition.current.onerror = (event: any) => {
      ...
      switch (event.error) {
        case 'network':
          errorMessage =
            'A network error occurred. Speech recognition requires an active internet connection. Please check your connection and try again.';
          break;
        case 'no-speech':
          errorMessage =
            "I didn't hear anything. Please ensure your microphone is working and speak clearly. Tap the mic to try again.";
          break;
        case 'not-allowed':
        case 'service-not-allowed':
          errorMessage =
            'Permission to use the microphone was denied. Please allow microphone access for this site in your browser settings to continue.';
  ...
  const speak = (text: string) => {
    ...
    utterance.onerror = e => {
      ...
      addMessage(
        'system',
        "Sorry, an error occurred and I couldn't speak the response."
      );
      setIsSpeaking(false);
    };
  ...
  const startInterview = async () => {
    if (!navigator.onLine) {
      setError(
        'You appear to be offline. An internet connection is required for the mock interview.'
      );
      return;
    }
    ...
    addMessage(
      'system',
      `Starting interview for ${job.title} at ${job.company}...`
    );
  ...
  const sendToAI = async (message: string) => {
    ...
    try {
      ...
      const errorText =
        'Sorry, I encountered an error. The interview will now end.';
  ...
  if (!isInterviewing) {
    return (
      <div className="ai-tool-panel">
        ...
        <p className="placeholder-text">
          Practice your interview skills with an AI interviewer. Your
          conversation will be voice-based.
        </p>
        <div className="generator-controls vertical">
          <select
            value={selectedJobId ?? ''}
            onChange={e =>
              updateCurrentToolState({ selectedJobId: Number(e.target.value) })
            }
            disabled={jobs.length === 0}
            aria-label="Select job for interview prep"
          >
            <option value="" disabled>
              Select a job...
            </option>
          ...
          <select
            ...
            onChange={e =>
              updateCurrentToolState({
                selectedResumeId: Number(e.target.value),
              })
            }
            disabled={resumes.length === 0}
            aria-label="Select resume for interview prep"
          >
            <option value="" disabled>
              Select a resume...
            </option>
          ...
          <select
            ...
            aria-label="Select AI interviewer voice"
          >
            <option value="" disabled>
              Select a voice...
            </option>
        ...
        >
          <button
            ...
            disabled={!selectedJobId || !selectedResumeId || !selectedVoiceURI}
            className="button-start-interview"
          >
            Start Interview
          </button>
        </div>
...
const AiToolsView = ({
  ...
  const updateCurrentToolState = (
    updates: Partial<(typeof toolStates)[ToolTab]>
  ) => {
    setToolStates(prev => ({
      ...prev,
  ...
  useEffect(() => {
    ...
    if (currentState.selectedJobId === null) {
      const firstJobWithDesc = jobs.find(j => j.description);
      if (firstJobWithDesc)
        updateCurrentToolState({ selectedJobId: firstJobWithDesc.id });
      else if (jobs.length > 0)
        updateCurrentToolState({ selectedJobId: jobs[0].id });
    }
  ...
  const handleCheckResume = async () => {
    ...
    if (!job || !resume || !job.description) {
      updateCurrentToolState({
        resumeCheckResult: {
          score: 0,
          keywords: ['Please select a job (with a description) and a resume.'],
        },
      });
      return;
    }
    ...
    try {
      ...
      const overallScore =
        resultJson.overall_match_percentage || resultJson.score || 0;
      ...
      const allRecommendations =
        recommendations || resultJson.missing_keywords || [];
      ...
      const jobReq =
        transparentAnalysis.job_requirements || resultJson.job_requirements;
      if (jobReq && Object.keys(jobReq).length > 0) {
        ...
        if (req.years_experience)
          analysisText += `Experience: ${req.years_experience} years\n`;
        else analysisText += `Experience: Not specified\n`;
        if (req.education_required)
          analysisText += `Education: ${req.education_required}\n`;
      ...
      // Add resume summary
      const resumeSum =
        transparentAnalysis.resume_summary || resultJson.resume_summary;
      if (resumeSum && Object.keys(resumeSum).length > 0) {
        const res = resumeSum;
        analysisText += `**Resume Summary:**\n`;
        if (res.total_experience_years)
          analysisText += `Experience: ${res.total_experience_years} years\n`;
        else analysisText += `Experience: Not specified\n`;
        if (res.education_level)
          analysisText += `Education: ${res.education_level}\n`;
        else analysisText += `Education: Not specified\n`;
        if (res.current_job_title)
          analysisText += `Current Role: ${res.current_job_title}\n`;
      ...
      if (ka && Object.keys(ka).length > 0) {
        ...
        if (ka.job_keywords && ka.job_keywords.length > 0) {
          ...
          ka.job_keywords.slice(0, 10).forEach((kw: any, i: number) => {
            const importance =
              kw.importance_score >= 7
                ? '●'
                : kw.importance_score >= 5
                  ? '○'
                  : '·';
        ...
        if (ka.resume_keywords && ka.resume_keywords.length > 0) {
          ...
          ka.resume_keywords.slice(0, 10).forEach((kw: any, i: number) => {
            const strength =
              kw.relevance_score >= 12
                ? '●'
                : kw.relevance_score >= 8
                  ? '○'
                  : '·';
      ...
      if (allRecommendations && allRecommendations.length > 0) {
        ...
        if (typeof allRecommendations[0] === 'object') {
          allRecommendations.forEach((rec: any, i: number) => {
            ...
            if (rec.keyword_to_add)
              analysisText += `**Add to resume:** ${rec.keyword_to_add}\n`;
          ...
          allRecommendations
            .slice(0, 5)
            .forEach((keyword: string, i: number) => {
      ...
      try {
        ...
        if (uniqueMissingSkills.length > 0) {
          ...
          const techPromptResponse = await fetch(
            '/prompts/technology-descriptions-prompt.txt'
          );
          const techPromptTemplate = await techPromptResponse.text();

          console.log(
            'Prompt template loaded, length:',
            techPromptTemplate.length
          );
          ...
          console.log(
            'Second LLM call completed, response length:',
            techResponse.text.length
          );
          ...
          if (techResponseText.includes('```json')) {
            techResponseText = techResponseText
              .replace(/```json\n?/g, '')
              .replace(/```\n?/g, '');
          ...
          try {
            ...
            console.error(
              'Skill descriptions JSON parse error:',
              techParseError
            );
      ...
      updateCurrentToolState({
        resumeCheckResult: {
          score: 0,
          keywords: ['Error: Could not perform analysis. Please try again.'],
        },
      });
    } finally {
      setIsLoading(false);
  ...
  const handleGenerateLetter = async () => {
    ...
    if (!job || !resume) {
      updateCurrentToolState({
        resultText:
          'Please select a job and a resume to generate a cover letter.',
      });
      return;
    }
    ...
    if (!job.description || job.description.trim().length < 50) {
      updateCurrentToolState({
        resultText:
          'Error: Job description is too short or missing. Please select a job with a detailed description.',
      });
      return;
    }
    ...
    const contact = job.contactIds?.[0]
      ? crmContacts.find(c => c.id === job.contactIds[0])
      : null;
    ...
    try {
      ...
      console.error('Cover letter generation failed:', error);
      updateCurrentToolState({
        resultText: 'Error: Could not generate cover letter. Please try again.',
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDeepResearch = async () => {
    ...
    if (!job) {
      updateCurrentToolState({
        resultText: 'Please select a job to research.',
      });
      return;
    }
    setIsLoading(true);
...
`;
    ...
    try {
      ...
      updateCurrentToolState({
        resultText:
          'Error: Could not generate the research report. The topic may be restricted or another error occurred. Please try again.',
      });
    } finally {
      setIsLoading(false);
    }
      ...
      case 'Resume Checker':
        return (
          <div className="ai-tool-panel resume-checker-tool">
            ...
            <p className="placeholder-text">
              Get an ATS-friendly score and find missing keywords.
            </p>
            <div className="generator-controls">
              <select
                ...
                disabled={jobs.length === 0}
                aria-label="Select job for resume check"
              >
                <option value="" disabled>
                  Select a job...
                </option>
              ...
              <select
                ...
                disabled={resumes.length === 0}
                aria-label="Select resume for resume check"
              >
                <option value="" disabled>
                  Select a resume...
                </option>
              ...
              <button
                ...
                disabled={isLoading || !selectedJobId || !selectedResumeId}
              >
      ...
      case 'Cover Letter':
        return (
          <div className="ai-tool-panel">
            ...
            <p className="placeholder-text">
              Generate a tailored cover letter based on a job and resume.
            </p>
            <div className="generator-controls">
              <select
                value={selectedJobId ?? ''}
                ...
                disabled={jobs.length === 0}
                aria-label="Select job for cover letter"
              >
                <option value="" disabled>
                  Select a job...
                </option>
              ...
              <select
                ...
                disabled={resumes.length === 0}
                aria-label="Select resume for cover letter"
              >
                <option value="" disabled>
                  Select a resume...
                </option>
            ...
            <div className="ai-result-box large">
              {isLoading && (
                <div className="loading-indicator">
                  <span>AI is writing your letter...</span>
                </div>
              )}
              {resultText && (
      ...
      case 'Deep Research':
        return (
          <div className="ai-tool-panel">
            <h3>Deep Research</h3>
            <p className="placeholder-text">
              Get a detailed, internet-powered report on a company and role.
            </p>
            <div className="generator-controls">
              <select
                value={selectedJobId ?? ''}
                ...
                disabled={jobs.length === 0}
                aria-label="Select job for deep research"
              >
                <option value="" disabled>
                  Select a job...
                </option>
      ...
      case 'General Assistant':
        return (
          <GeneralAssistantTool
            ...
            isAgentMixMode={isAgentMixMode}
            setIsAgentMixMode={setIsAgentMixMode}
          />
        );
      default:
        return null;
  ...
  return (
    <div className="ai-tools-view">
      ...
      <div className="ai-tool-content">{renderTool()}</div>
    </div>
  );
};
...
const ResumePreview = ({ resume }: { resume: Resume }) => {
  ...
  return (
    <div
      ...
      ref={previewRef}
      style={{ '--page-count': pages } as React.CSSProperties}
    >
        ...
        <section className="preview-section">
          ...
          {resume.experience.map(exp => (
            <div key={exp.id} className="preview-job">
              <h3>
                {exp.role || 'Job Title'}{' '}
                <span>at {exp.company || 'Company'}</span>
              </h3>
...
const ResumeEditor = ({
  ...
  const handleExport = async (format: 'pdf' | 'doc') => {
    const safeFilename = formData.name
      .replace(/[^a-z0-9]/gi, '_')
      .toLowerCase();
    ...
    const paperElement = document.querySelector(
      '#pdf-export-node .resume-preview-paper'
    ) as HTMLElement;
    if (!paperElement) {
          ...
          {(formData.customSections || []).map((sec, index) => (
            <div key={sec.id} className="dynamic-form-item">
              ...
              <div className="form-grid">
                ...
                <div className="form-group full-width">
                  <textarea
                    placeholder="Section content (use bullet points)..."
                    value={sec.content}
                    onChange={e =>
...
const ResumesView = ({
  ...
  const handleDeleteResume = () => {
    ...
    if (
      window.confirm(`Are you sure you want to delete this resume version?`)
    ) {
    ...
    <div className="resume-builder-layout">
      <div className="resume-list-panel">
        ...
        <ul className="resume-version-list">
          ...
          {resumes.length === 0 && (
            <p className="placeholder-text">No resumes created yet.</p>
          )}
        </ul>
        <div className="resume-list-actions">
          <button onClick={handleDuplicateResume} disabled={!selectedResumeId}>
            Duplicate
          </button>
          <button
...
const SettingsView = ({
  ...
  const handleToolToggle = (
    ...
    const currentToolState =
      localSettings.agents[agentKey].tools[toolId]?.enabled || false;
    handleNestedChange('agents', agentKey, {
      ...localSettings.agents[agentKey],
      tools: {
  ...
  const handleDeleteAccount = async () => {
    if (
      window.confirm(
        'Are you absolutely sure? This will permanently delete all your data from the database. This action cannot be undone.'
      )
    ) {
      if (
        window.confirm(
          'FINAL CONFIRMATION: Please confirm you want to delete all data.'
        )
      ) {
        try {
            ...
            <div className="form-grid two-cols">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  type="number"
                  min="0"
                  value={localSettings.profile.weeklyGoal}
                  onChange={e =>
                    handleNestedChange(
                      'profile',
                      'weeklyGoal',
              ...
              <div className="form-group full-width">
                ...
                <textarea
                  id="masterSkills"
                  value={localSettings.profile.masterSkills}
                  onChange={e =>
                    handleNestedChange(
                      'profile',
                      'masterSkills',
                ...
                ></textarea>
                <p className="placeholder-text">
                  Enter skills, comma-separated. Used by AI tools.
                </p>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'AI & Prompts' && (
          <>
            <div className="settings-card">
              ...
              <div className="form-grid">
                <div className="form-group">
                  <label htmlFor="resumeCheckerPrompt">
                    Resume Checker Prompt (Read-only)
                  </label>
                  <textarea
                  ...
                  ></textarea>
                  <p className="placeholder-text">
                    This prompt is loaded from
                    prompts/resume-checker-prompt.txt. Edit the file to modify.
                  </p>
                </div>
                <div className="form-group">
                  <label htmlFor="coverLetterPrompt">
                    Cover Letter Prompt (Read-only)
                  </label>
                  <textarea
                  ...
                  ></textarea>
                  <p className="placeholder-text">
                    This prompt is loaded from prompts/cover-letter-prompt.txt.
                    Edit the file to modify.
                  </p>
                </div>
                <div className="form-group">
                  <label htmlFor="interviewQuestionsPrompt">
                    Interview Questions Prompt
                  </label>
                  <textarea
                    id="interviewQuestionsPrompt"
                    value={localSettings.prompts.interviewQuestions}
                    onChange={e =>
                      handleNestedChange(
                        'prompts',
                        'interviewQuestions',
                  ...
                  ></textarea>
                  <p className="placeholder-text">
                    Use placeholders like {'{{JOB_TITLE}}'}, {'{{COMPANY}}'},{' '}
                    {'{{JOB_DESCRIPTION}}'}, {'{{RESUME_CONTENT}}'} in the
                    prompt.
                  </p>
                </div>
                <div className="form-group">
                  <label htmlFor="mixAgentsPrompt">
                    AI Assistant (Mix Agents) Prompt
                  </label>
                  <textarea
                    id="mixAgentsPrompt"
                    value={localSettings.prompts.mixAgents}
                    onChange={e =>
                      handleNestedChange('prompts', 'mixAgents', e.target.value)
                    }
                    rows={5}
                  ></textarea>
                </div>
                  ...
                  <div key={key} className="agent-config-card">
                    ...
                    <div className="tool-grid">
                      {availableTools.map(tool => (
                        <div
                          key={tool.id}
                          className={`tool-tile ${agent.tools[tool.id]?.enabled ? 'active' : ''}`}
                          onClick={() =>
                            handleToolToggle(
                              key as keyof Settings['agents'],
                              tool.id
            ...
            <div className="subscription-plan-grid">
              <div className="subscription-plan-card current">
                ...
                <p>
                  Status:{' '}
                  <span
                    className={`status-${localSettings.subscription.status.toLowerCase()}`}
                  >
                    {localSettings.subscription.status}
                  </span>
                </p>
                {localSettings.subscription.status === 'Active' && (
                  <p>
                    Renews on:{' '}
                    {new Date(
                      localSettings.subscription.nextBillingDate
                    ).toLocaleDateString()}
                  </p>
                )}
        ...
        {activeTab === 'Data & Privacy' && (
          <div className="settings-card">
            ...
            <div className="privacy-control">
              ...
              <div className="toggle-container">
                <p className="placeholder-text">
                  Help improve the app by sharing anonymized data for benchmarks
                ...
                <label className="toggle-switch">
                  <input
                    id="shareData"
                    type="checkbox"
                    checked={localSettings.privacy.shareAnonymizedData}
                    onChange={() =>
                      handleNestedChange(
                        'privacy',
                        'shareAnonymizedData',
                        !localSettings.privacy.shareAnonymizedData
                      )
                    }
                  />
                  <span className="slider"></span>
            ...
            <div className="data-action">
              ...
              <p className="placeholder-text">
                Download a JSON file of all your jobs, contacts, resumes, and
            ...
            <div className="data-action">
              ...
              <p className="placeholder-text">
                Permanently delete your account and all associated data from the
...
const AnalyticsView = ({
  ...
  const pipelineStats = React.useMemo(() => {
    const appliedStatuses: JobStatus[] = [
      'Applied',
      'Screening',
      'Interviewing',
      'Test Task',
    ...
    const appliedCount = jobs.filter(j =>
      appliedStatuses.includes(j.status)
    ).length;
    const interviewingCount = jobs.filter(j =>
      interviewStatuses.includes(j.status)
    ).length;
    const offerCount = jobs.filter(j => j.status === 'Offer').length;

    const appliedToInterviewRate =
      appliedCount > 0 ? interviewingCount / appliedCount : 0;
    const interviewToOfferRate =
      interviewingCount > 0 ? offerCount / interviewingCount : 0;
  ...
  const resumePerformance = React.useMemo(() => {
    const performance = new Map<
      number,
      { name: string; applied: number; interviews: number }
    >();
    ...
    jobs.forEach(job => {
      if (job.selectedResumeId) {
        if (!performance.has(job.selectedResumeId)) {
          ...
          performance.set(job.selectedResumeId, {
            name: resume?.name || `Resume ID ${job.selectedResumeId}`,
            applied: 0,
            interviews: 0,
          });
        }
  ...
  const sourceEffectiveness = React.useMemo(() => {
    ...
    return Array.from(effectiveness.entries())
      .map(([source, data]) => ({
        source,
        rate: data.applied > 0 ? data.interviews / data.applied : 0,
      }))
      .sort((a, b) => b.rate - a.rate);
  }, [jobs]);
          ...
          <div className="funnel-container">
            ...
            <div className="funnel-arrow">
              <div className="funnel-rate">
                {(pipelineStats.appliedToInterviewRate * 100).toFixed(1)}%
              </div>
              →
            </div>
            <div className="funnel-stage">
              <div className="funnel-value">
                {pipelineStats.interviewingCount}
              </div>
              <div className="funnel-label">Interviews</div>
            </div>
            <div className="funnel-arrow">
              <div className="funnel-rate">
                {(pipelineStats.interviewToOfferRate * 100).toFixed(1)}%
              </div>
              →
            </div>
            <div className="funnel-stage">
              ...
              <div key={i} className="chart-bar-col">
                <div className="bar-wrapper">
                  <div
                    className="bar"
                    style={{
                      height: `${(week.count / Math.max(...weeklyActivity.map(w => w.count), 1)) * 100}%`,
                    }}
        ...
        <div className="analytics-widget">
          ...
          <div className="bar-chart-horizontal">
            ...
            {resumePerformance.length === 0 && (
              <p className="placeholder-text">
                Apply to jobs with a linked resume to see performance.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="analytics-widget">
          ...
          <div className="bar-chart-horizontal">
            ...
            {sourceEffectiveness.length === 0 && (
              <p className="placeholder-text">
                Add sources to your jobs to see which are most effective.
              </p>
            )}
          </div>
        </div>
...
const App = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [jobsData, resumesData, contactsData, eventsData, settingsData] =
          await Promise.all([
            apiService.getJobs(),
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      sender: 'ai',
      text: 'Hello! Use the tools above for specific tasks, or ask me a general question here.',
    },
  ]);
  ...
  const handleSendMessage = async (userInput: string, file?: File) => {
    ...
    if (isAgentMixMode) {
      const agentDescriptions = Object.values(settings.agents)
        .map(agent => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'Dashboard':
        return (
          <DashboardView
            ...
            onJobDoubleClick={handleOpenJobModal}
          />
        );
      case 'Job Tracker':
        return (
      ...
      case 'Analytics':
        return <AnalyticsView jobs={jobs} resumes={resumes} />;
      case 'Preferences':
        return (
