...
class DatabaseManager {
    ...
    createJob(job) {
        ...
        const result = stmt.run(
            job.title, job.company, job.status, job.description, job.selectedResumeId,
            job.coverLetter, job.location, job.sourceUrl, job.source, job.dateAdded,
            job.applicationDate, JSON.stringify(job.contactIds || []),
            job.portfolioUrl, JSON.stringify(job.communicationLog || []),
            job.questionsForInterviewer, JSON.stringify(job.salaryInfo || {}),
            job.notes, job.priority, job.interestLevel, JSON.stringify(job.tags || []),
            job.nextStep, job.nextStepDate
        );
    ...
    updateJob(job) {
        ...
        stmt.run(
            job.title, job.company, job.status, job.description, job.selectedResumeId,
            job.coverLetter, job.location, job.sourceUrl, job.source, job.applicationDate,
            JSON.stringify(job.contactIds || []), job.portfolioUrl,
            JSON.stringify(job.communicationLog || []), job.questionsForInterviewer,
            JSON.stringify(job.salaryInfo || {}), job.notes, job.priority,
            job.interestLevel, JSON.stringify(job.tags || []), job.nextStep,
            job.nextStepDate, job.id
        );
    ...
    getAllContacts() {
        const stmt = this.db.prepare('SELECT * FROM contacts ORDER BY lastInteraction DESC');
        return stmt.all();
    }
    ...
    createContact(contact) {
        ...
        const result = stmt.run(
            contact.name, contact.role, contact.company, contact.email, contact.phone,
            contact.linkedinUrl, contact.source, contact.notes, contact.dateAdded,
            JSON.stringify(contact.tags || []), contact.lastInteraction
        );
    ...
    updateContact(contact) {
        ...
        stmt.run(
            contact.name, contact.role, contact.company, contact.email, contact.phone,
            contact.linkedinUrl, contact.source, contact.notes,
            JSON.stringify(contact.tags || []), contact.lastInteraction, contact.id
        );
    ...
    getAllEvents() {
        const stmt = this.db.prepare('SELECT * FROM calendar_events ORDER BY date ASC');
        return stmt.all();
    }
    ...
    createEvent(event) {
        ...
        const result = stmt.run(
            event.title, event.date, event.time, event.type, event.notes,
            event.jobId, event.contactId
        );
    ...
    updateEvent(event) {
        ...
        stmt.run(
            event.title, event.date, event.time, event.type, event.notes,
            event.jobId, event.contactId, event.id
        );
